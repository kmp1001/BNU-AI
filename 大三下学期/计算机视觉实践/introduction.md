...
1. **分层递进**：  
   - 基础：数字图像与 OpenCV 编程  
   - 进阶：传统视觉算法（分割、检测、跟踪）  
   - 高级：深度学习与系统集成（DNN、OCR、交互、扩展）  

2. **案例＋实战**：每个模块配至少一个真实场景小项目，用“做”来加深理解。  

3. **开源驱动**：引导学生复用/阅读/改写开源代码，鼓励在项目中引用、改进公开方法并写明出处。  

4. **小组协作**：项目以小组为单位推进，组内分工明确；组间互评促成反思与学习。  

5. **前沿引入**：周期性补充当前视觉领域新技术（如轻量化、基础模型、迁移与微调策略、视频生成/多模态等）作为拓展讲座或选读材料。  

6. **学习记录**：建议学生保留学习日志与错误-修复记录，形成“元学习”能力。
...
project-name/
├── notebooks/
│   └── experiment\_x.ipynb        # 含代码、结果、注释
├── report/
│   └── final\_report.pdf         # 开题/结题补充（可选精炼）
├── slides/
│   └── presentation.pdf         # 汇报用 PPT
├── src/                        # 若有模块化代码
...
- Notebook 要求：  
- 明确分块（数据加载、预处理、核心算法、结果展示）  
- 配合图示/可视化（例如：检测框、分割 mask、跟踪轨迹）  
- 加入简短文字说明当前模块做了什么、用了哪些方法、遇到的挑战与解决思路  
...
|------|----------------|
| 考勤 | 10%（平时） |
| 平时作业 | 40%（Notebook 实验为主，含注释/思路） |
| 中期汇报 | 10%（原大作业中期进展，纳入平时） |
| 开题汇报 | 10%（期末部分） |
| 结题汇报 + 代码 & 实现 | 30%（期末核心） |
| 组间互评 | 10%（辅助影响） |
| 教师评价 | 80%（在项目评价中占主导） |
| 附加分 | 最高可加 2 分（按规则融入平时/期末） |

## 八、补充说明

- 本课程**不设笔试**，所有能力通过“动手/汇报/项目”体现。  
- 鼓励使用版本控制（Git/GitHub），所有提交推荐包括可复现说明（如何跑、依赖、预期输出）。  
- 期末大作业是综合能力体现，设计时应兼顾**创新、工程实现、可解释性与汇报表达**。  
- 若需参赛（学科竞赛等），可将期末项目延展为竞赛版本，优秀作品可加分或推荐。

```

如果你希望我基于这个再做一个 **模板 `README.md` 仓库结构**（含 issue 模板、作业提交流程、评分 rubric 表格、PR/Review 指南等），我可以继续为你生成适配的版本。你想先要哪部分？
